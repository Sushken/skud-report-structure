...
class AddTimeOfWork(QtCore.QThread):
    ...
    def __init__(self, nameOfOutFile, nameOfInFile, count, value):
        super().__init__()
        ...
        while i <= sheetOut.max_row:
            if sheetOut.cell(row=i, column=9).value is not None:
                if len(str(self.value)) == 2:
                    if sheetOut.cell(row=i, column=9).value.startswith(str(self.value)):
                        sheetOut.cell(row=i, column=9).value = ""
                        sheetOut.cell(row=i, column=10).value = ""
                else:
                    if sheetOut.cell(row=i, column=9).value.startswith("0" + str(self.value)):
                        sheetOut.cell(row=i, column=9).value = ""
                        sheetOut.cell(row=i, column=10).value = ""
            i += 1
        wbOut.save(filename=self.nameOfOutFile)
